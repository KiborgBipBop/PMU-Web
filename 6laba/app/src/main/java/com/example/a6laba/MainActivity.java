...
public class MainActivity extends AppCompatActivity
{
    ...
    { //onCreate задаёт начальную установку параметров при инициализации активности
            ...
            public void onClick(View v)
            {
                ...
                if (chLast >= '0' && chLast <= '9' || chLast == '.')
                {
                    while (chLast >= '0' && chLast <= '9' || chLast == '.')
                    {
                        ...
                        {//берём предпоследний символ, его элемент
                ...
                if (can_dot)
                {
                    ...
                    if (!val.isEmpty() && (val.charAt(lastPost) < '0' || val.charAt(lastPost) > '9'))
                        result = text_view_main.getText() + "0.";
        ...
        bc.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                { //проверяем с конца входят ли данные символы
    ...
    public static double eval(final String str)
    {
        return new Object()
        {
            ...
            { //соответствует ли текущий символ тому, что мы ему передали , если да - True
                while (ch == ' ') nextChar();
                if (ch == charToEat)
                {
                    nextChar();
            ...
            double parseFactor()
            {
                ...
                { // скобочки
                    x = parseExpression();
                    eat(')');
                } else if ((ch >= '0' && ch <= '9') || ch == '.' || ch == 'E')
                { // циферки
                    while ((ch >= '0' && ch <= '9') || ch == '.' || ch == 'E') nextChar();
                    x = Double.parseDouble(str.substring(startPos, this.pos));
                } else if (ch >= 'a' && ch <= 'z')
                { // символы для распознания функций
                    while (ch >= 'a' && ch <= 'z') nextChar();
                    String func = str.substring(startPos, this.pos);
                    if (str.length() == this.pos)
                        throw new RuntimeException("Not set last element");
                    x = parseFactor();
